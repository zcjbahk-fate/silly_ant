!function(){
    ...
    // [DOM安全替换] 核心修改点：剥去HTML直接在原地检验，不让序号错位
    function w(html, text, mid){
        var $wrapper = $('<div>').html(html);
        $wrapper.find('.corpse-report').remove();

        var newHtml = $wrapper.html();
        var idx = 0;

        // 获取标签块内容
        var htmlReg = /\[\[corpse_data\]\]((?:(?!\[\[corpse_data\]\]|\[\[\/corpse_data\]\])[\s\S])*?)\[\[\/corpse_data\]\]/g;

        newHtml = newHtml.replace(htmlReg, function(matchStr, innerMatch) {
            // 利用包裹元素抽取干净文本，滤掉乱入的 <br> 或 <p>
            var pureTxt = $('<div>').html(innerMatch).text().trim();

            // 当场检查，如果是合法的才转成UI，如果不合法，直接静默消除这段乱码
            if (isBlockValid(pureTxt)) {
                idx++;
                var p = x(pureTxt);
                return p ? h(p, mid, f(pureTxt) + '_' + idx) : '';
            } else {
                return ''; // 这是个错误标签，让它灰飞烟灭
            }
        });
        ...
        return new Promise(function(r){
            !async function(){
                try{
                    ...
                    var i=n.html()||'', c=w(i, o.message, e);
                    if(!c||c===i) { renderLocks[e] = false; return r(!1); }
    ...
    function M(){
        try{
            ...
            $(window).on('pagehide',function(){ t(); $('#'+r).remove() });
        }catch(s){}
    }
